...
export function createRufus({content,body,unlock,close,developer=()=>false,available=()=>true}){
 ...
 function render(){
  ...
  const p=progressFor(state,t);body(`RUFUS · ${t.format.toUpperCase()}`,t.title);if(developer())content.append(button('Überspringen',()=>{p.developerSkipped=true;p.answer=t.type==='repair'?{index:t.wrongIndex,text:t.answer}:t.type==='translation'?t.translation:JSON.parse(JSON.stringify(t.answer));if(t.type==='translation'){p.example=true;p.review=t.criteria.map(()=>true);}p.done=true;save();if(t.memory)unlock([t.memory]);const next=rufusTasks.slice(rufusTasks.indexOf(t)+1).find(x=>available(x)&&!state.tasks[x.id]?.done);state.selected=next?.id||null;save();render();},'developer-skip'));content.append(button('‹ Alle Tafeln',menu,'text-button'),el('p',t.context,'scene-context'));speech(t);document.querySelector('#quest-topic')?.replaceChildren(el('span','Wiederholung · '+t.topic));
